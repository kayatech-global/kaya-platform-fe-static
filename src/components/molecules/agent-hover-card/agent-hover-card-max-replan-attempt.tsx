...
export const AgentHoverCardMaxReplanAttempt = ({ data }: IAgentHoverCardRagsProps) => {
    const agent = data as PlannerReplannerAgent;

    return (
        <div className="flex flex-col py-3">
            <p className="text-xs font-medium text-gray-700 dark:text-gray-100 mb-2">Max Replan Attempts</p>
            <div className="flex flex-col gap-y-2">
                <p className="text-xs font-regular text-gray-400 dark:text-gray-300">
                    {agent?.planConfig?.maxReplanAttempts ?? 3}
                </p>
            </div>
        </div>
    );
