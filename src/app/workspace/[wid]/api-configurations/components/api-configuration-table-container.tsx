'use client';

import React, { useState } from 'react';
import { ColumnDef, Row } from '@tanstack/react-table';
import { Trash2, Pencil, Download } from 'lucide-react';
import { Button, Input } from '@/components';
import DataTable from '@/components/molecules/table/data-table';
import { useForm } from 'react-hook-form';
import { cn, handleNoValue } from '@/lib/utils';
import { IAuthorization, IHeaderValues } from '@/models';
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/atoms/dialog';
import { useBreakpoint } from '@/hooks/use-breakpoints';

export interface ApiConfigurationData {
    defaultApiParameters: any;
    id: string;
    apiHeaders: IHeaderValues[];
    apiMethod: string;
    apiName: string;
    apiUrl: string;
    payloadFormat: string;
    search?: string;
    description?: string;
    authorization?: IAuthorization;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    payloadOutput?: any;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    promotedVariables?: any;
    isReadOnly?: boolean;
    concurrencyLimit?: number | null;
}

interface ApiConfigurationTableContainerProps {
    apiConfigurations: ApiConfigurationData[];
    onApiConfigurationFilter: (filter: ApiConfigurationData | null) => void;
    onNewButtonClick: () => void;
    onImportBulkClick: () => void;
    onEditButtonClick: (id: string) => void;
    onDelete: (id: string) => void;
    onRecentActivity: () => void;
}

const DeleteRecord = ({ row, onDelete }: { row: Row<ApiConfigurationData>; onDelete: (id: string) => void }) => {
    const [open, setOpen] = useState<boolean>(false);

    const handleDelete = () => {
        onDelete(row.original.id);
        setOpen(false);
    };

    return (
        <>
            <Button
                className={`w-full sm:w-max ${row.original.isReadOnly ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                variant="link"
                size="icon"
                onClick={() => setOpen(true)}
                disabled={row.original.isReadOnly}
            >
                <Trash2
                    size={18}
                    className={cn('', {
                        'text-gray-300 dark:text-gray-600': row.original.isReadOnly,
                        'text-gray-500 dark:text-gray-200': !row.original.isReadOnly,
                    })}
                />
            </Button>
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent className="overflow-y-auto max-h-[80%]">
                    <DialogHeader>
                        <DialogTitle>
                            <p className="text-sm text-gray-500 dark:text-gray-400">
                                Are you sure, do you want to delete this?
                            </p>
                        </DialogTitle>
                    </DialogHeader>
                    <div className="flex justify-end gap-2 p-3">
                        <Button variant={'secondary'} size="sm" onClick={() => setOpen(false)}>
                            No
                        </Button>
                        <Button variant={'primary'} size="sm" onClick={handleDelete}>
                            Yes
                        </Button>
                    </div>
                </DialogContent>
            </Dialog>
        </>
    );
};

const generateColumns = (onEditButtonClick: (id: string) => void, onDelete: (id: string) => void) => {
    const columns: ColumnDef<ApiConfigurationData>[] = [
        {
            accessorKey: 'apiName',
            enableSorting: true,
            header() {
                return <div className="w-full text-left">API Name</div>;
            },
            cell({ row }) {
                return <div>{handleNoValue(row.getValue('apiName'))}</div>;
            },
        },
        {
            accessorKey: 'apiUrl',
            enableSorting: false,
            header() {
                return <div className="w-full text-left">API URL</div>;
            },
            cell({ row }) {
                return <div>{handleNoValue(row.getValue('apiUrl'))}</div>;
            },
        },
        {
            accessorKey: 'id',
            enableSorting: false,
            header() {
                return <div className="w-full text-left"></div>;
            },
            cell({ row }) {
                return (
                    <div className="flex items-center gap-x-4">
                        <DeleteRecord row={row} onDelete={onDelete} />
                        <Pencil
                            size={18}
                            className="text-gray-500 cursor-pointer dark:text-gray-200"
                            onClick={() => onEditButtonClick(row.getValue('id'))}
                        />
                    </div>
                );
            },
        },
    ];
    return columns;
};

export const ApiConfigurationTableContainer = ({
    apiConfigurations,
    onApiConfigurationFilter,
    onNewButtonClick,
    onImportBulkClick,
    onEditButtonClick,
    onDelete,
    onRecentActivity,
}: ApiConfigurationTableContainerProps) => {
    const { register, handleSubmit } = useForm<ApiConfigurationData>({ mode: 'onChange' });
    const [debounceTimer, setDebounceTimer] = useState<NodeJS.Timeout | null>(null);
    const { isMobile } = useBreakpoint();

    const onHandleSubmit = (data: ApiConfigurationData) => {
        if (debounceTimer) {
            clearTimeout(debounceTimer);
        }

        const timer = setTimeout(() => {
            onApiConfigurationFilter(data);
        }, 1000);
        setDebounceTimer(timer);
    };

    const columns = generateColumns(onEditButtonClick, onDelete);

    return (
        <div className="grid gap-8">
            <DataTable
                columns={columns}
                data={apiConfigurations}
                searchColumnName="workflow"
                showFooter
                defaultPageSize={isMobile ? 5 : 10}
                showTableSearch={false}
                manualSpan={true}
                tableHeader={
                    <div className="flex justify-between items-center w-full">
                        <Input
                            {...register('search')}
                            placeholder="Search by API Name"
                            className="max-w-sm"
                            onKeyUp={handleSubmit(onHandleSubmit)}
                        />
                        <div className="flex ml-2 justify-end items-center gap-4 w-full">
                            <Button size={'sm'} onClick={onNewButtonClick}>
                                New API
                            </Button>
                            <Button size={'sm'} variant={'primary'} onClick={onImportBulkClick}>
                                <Download size={18} className="mr-1" />
                                Import APIs
                            </Button>
                            <Button variant={'link'} size={'sm'} onClick={onRecentActivity} className="hidden">
                                Recent Activities
                            </Button>
                        </div>
                    </div>
                }
            />
        </div>
    );
};
