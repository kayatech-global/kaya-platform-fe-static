...
export interface ReRankingData extends IReRanking {
    search?: string;
}

interface ReRankingTableContainerProps {
    reRanking: ReRankingData[];
    providers: IProvider[];
    onReRankingFilter: (filter: ReRankingData | null) => void;
...
const DeleteRecord = ({ row, onDelete }: { row: Row<ReRankingData>; onDelete: (id: string) => void }) => {
...
const generateColumns = (onEditButtonClick: (id: string) => void, onDelete: (id: string) => void) => {
    const columns: ColumnDef<ReRankingData>[] = [
        {
            accessorKey: 'name',
            enableSorting: true,
            header() {
...
const ReRankingModelConfigurationFilter = ({
    providers,
    watch,
    register,
    reset,
    ...
    providers: IProvider[];
    watch: UseFormWatch<ReRankingData>;
    register: UseFormRegister<ReRankingData>;
    reset: UseFormReset<ReRankingData>;
    handleSubmit: UseFormHandleSubmit<ReRankingData>;
    onReRankingFilter: (filter: ReRankingData | null) => void;
}) => {
    ...
    const onHandleSubmit = (data: ReRankingData) => {
        onReRankingFilter(data);
        setOpen(false);
    };
    ...
    const clearFilter = () => {
        ...
        onReRankingFilter({ search: watch('search') } as ReRankingData);
        setOpen(false);
    };
...
export const ReRankingModelConfigurationTableContainer = ({
    reRanking,
    providers,
    onReRankingFilter,
    onNewButtonClick,
    ...
    onDelete,
    onRecentActivity,
}: ReRankingTableContainerProps) => {
    const { register, handleSubmit, reset, watch } = useForm<ReRankingData>({ mode: 'onChange' });
                ...
                tableHeader={
                    <div className="flex justify-between items-center w-full">
                        ...
                        <div className="flex ml-2 justify-end items-center gap-2 w-full">
                            <ReRankingModelConfigurationFilter
                                providers={providers}
                                watch={watch}
                                register={register}
                                reset={reset}
