...
export interface GuardrailsApiConfigurationData {
    id: string;
    ...
    payloadOutput?: any;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    promotedVariables?: any;
    isReadOnly?: boolean;
}

interface GuardrailsApiConfigurationTableContainerProps {
    guardrailsApiConfigurations: GuardrailsApiConfigurationData[];
    onGuardrailsApiConfigurationFilter: (filter: GuardrailsApiConfigurationData | null) => void;
...
const DeleteRecord = ({
    ...
    row: Row<GuardrailsApiConfigurationData>;
    onDelete: (id: string) => void;
}) => {
                    ...
                    <DialogHeader>
                        <DialogTitle>
                            <p className="text-sm text-gray-500 dark:text-gray-400">
                                Are you sure, do you want to delete this Guardrails API configuration?
                            </p>
                        </DialogTitle>
                    </DialogHeader>
...
const generateColumns = (onEditButtonClick: (id: string) => void, onDelete: (id: string) => void) => {
    const columns: ColumnDef<GuardrailsApiConfigurationData>[] = [
        {
            accessorKey: 'apiName',
            enableSorting: true,
            header() {
                return <div className="w-full text-left">API Name</div>;
            },
            cell({ row }) {
                return <div>{handleNoValue(row.getValue('apiName'))}</div>;
            },
        },
        {
            accessorKey: 'apiUrl',
            enableSorting: false,
            header() {
                return <div className="w-full text-left">API URL</div>;
            },
            cell({ row }) {
                return <div>{handleNoValue(row.getValue('apiUrl'))}</div>;
            },
        },
        {
            accessorKey: 'id',
...
export const GuardrailsApiConfigurationTableContainer = ({
    guardrailsApiConfigurations,
    onGuardrailsApiConfigurationFilter,
    onNewButtonClick,
    onEditButtonClick,
    onDelete,
    onRecentActivity,
}: GuardrailsApiConfigurationTableContainerProps) => {
    const { register, handleSubmit } = useForm<GuardrailsApiConfigurationData>({ mode: 'onChange' });
    const [debounceTimer, setDebounceTimer] = useState<NodeJS.Timeout | null>(null);
    const { isMobile } = useBreakpoint();

    const onHandleSubmit = (data: GuardrailsApiConfigurationData) => {
        if (debounceTimer) {
            clearTimeout(debounceTimer);
        }

        const timer = setTimeout(() => {
            onGuardrailsApiConfigurationFilter(data);
        }, 1000);
    ...
    return (
        <div className="grid gap-8">
            <DataTable
                columns={columns}
                data={guardrailsApiConfigurations}
