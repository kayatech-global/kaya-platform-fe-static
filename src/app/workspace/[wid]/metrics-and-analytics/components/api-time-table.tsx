...
import { ApiTimeData, IApiExecutionFilters } from '@/models';
import { useForm } from 'react-hook-form';
import { handleNoValue } from '@/lib/utils';

interface ApiTimeTableProps {
    apiExecutions: ApiTimeData[];
    onApiExecutionFilter: (filter: IApiExecutionFilters | null) => void;
}

const columns: ColumnDef<ApiTimeData>[] = [
    {
        accessorKey: 'api',
        enableSorting: true,
        header() {
            return <div className="w-full text-left">API</div>;
        },
        cell({ row }) {
            return <div>{handleNoValue(row.getValue('api'))}</div>;
        },
    },
    {
    ...
    },
    {
        ...
        cell({ row }) {
            return <div className="w-full text-center">{handleNoValue(row.getValue('failureCount'))}</div>;
        },
    },
    {
        header: 'Time Taken',
        columns: [
            {
                ...
                header: 'Average',
                cell({ row }) {
                    return <div className="w-full text-center">{handleNoValue(row.getValue('average'))}</div>;
                },
            },
            {
                accessorKey: 'longest',
...
const ApiTimeFilter = ({
    ...
    const {
        ...
        formState: { errors },
        trigger,
    } = useForm<IApiExecutionFilters>({ mode: 'onChange' });
    ...
    const onHandleSubmit = (data: IApiExecutionFilters) => {
        onApiExecutionFilter(data);
        setOpen(false);
    };

    const clearFilter = () => {
        reset();
        onApiExecutionFilter(null);
    };
            ...
            >
                <div className="flex flex-col gap-y-6">
                    <div className="form-container flex flex-col gap-y-4">
                        ...
                        <div className="flex flex-col gap-y-2">
                            <div>
                                <p className="text-sm text-gray-800 font-normal dark:text-gray-200">Failure Count</p>
                            </div>
                            <div className="flex items-center gap-x-3">
                                <Input
                                    {...register('failureCount.min', { valueAsNumber: true })}
                                    placeholder="Min"
                                    type="number"
                                    onBlur={() => onMinValueChange('failureCount')}
                                    onInput={handleInputChange}
                                />
                                <Input
                                    {...register('failureCount.max', {
                                        valueAsNumber: true,
                                        validate: value => validateMaxGreaterThanMin(value, 'failureCount'),
                                    })}
                                    isDestructive={!!errors.failureCount?.max?.message}
                                    placeholder="Max"
                                    type="number"
                                    onInput={handleInputChange}
                                />
                            </div>
                            {errors?.failureCount?.max && (
                                <p className="text-red-500 text-xs">{errors.failureCount.max.message}</p>
                            )}
                        </div>
                        <div className="flex flex-col gap-y-2">
                            <div>
...
export const ApiTimeTable = ({ apiExecutions, onApiExecutionFilter }: ApiTimeTableProps) => {
    return (
        <DataTable
            columns={columns}
            data={apiExecutions}
            ...
            tableHeader={
                <div className="flex justify-between items-center w-full">
                    <p className="text-sm font-bold text-gray-700 dark:text-gray-200">API Time</p>
                    <ApiTimeFilter onApiExecutionFilter={onApiExecutionFilter} />
                </div>
            }
        />
    );
