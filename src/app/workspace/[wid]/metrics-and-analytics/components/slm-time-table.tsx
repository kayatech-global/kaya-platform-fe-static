...
import { ISLMExecutionFilters } from '@/models';
import { useForm } from 'react-hook-form';
import { handleNoValue } from '@/lib/utils';

export interface SLMTimeData {
    id: string;
    slm: string;
    timeAverage: string;
    timeLongest: string;
...
interface SLMTimeTableProps {
    slmExecutions: SLMTimeData[];
    onSLMExecutionFilter: (filter: ISLMExecutionFilters | null) => void;
}

const columns: ColumnDef<SLMTimeData>[] = [
    {
        accessorKey: 'slm',
        enableSorting: true,
        header() {
            return <div className="w-full text-left">SLM</div>;
        },
        cell({ row }) {
            return <div>{handleNoValue(row.getValue('slm'))}</div>;
        },
    },
    {
        header: 'Time Taken',
    ...
    },
    {
        ...
        columns: [
            {
                accessorKey: 'slmAverage',
                enableSorting: false,
                header: 'Average',
                cell({ row }) {
                    return <div className="w-full text-center">{handleNoValue(row.getValue('slmAverage'))}</div>;
                },
            },
            {
                accessorKey: 'slmMost',
                ...
                header: 'Most',
                cell({ row }) {
                    return <div className="w-full text-center">{handleNoValue(row.getValue('slmMost'))}</div>;
                },
            },
        ],
    },
...
const SLMTimeFilter = ({
    ...
    const {
        ...
        formState: { errors },
        trigger,
    } = useForm<ISLMExecutionFilters>({ mode: 'onChange' });
    ...
    const onHandleSubmit = (data: ISLMExecutionFilters) => {
        onSLMExecutionFilter(data);
        setOpen(false);
    };

    const clearFilter = () => {
        reset();
        onSLMExecutionFilter(null);
    };
                ...
                <div className="flex flex-col gap-y-6">
                    <div className="form-container flex flex-col gap-y-4">
                        ...
                        <div className="flex flex-col gap-y-2">
                            ...
                            <div className="flex items-center gap-x-3">
                                <Input
                                    {...register('averageSlmTokens.min', { valueAsNumber: true })}
                                    placeholder="Min"
                                    type="number"
                                    onBlur={() => onMinValueChange('averageSlmTokens')}
                                    onInput={handleInputChange}
                                />
                                <Input
                                    {...register('averageSlmTokens.max', {
                                        valueAsNumber: true,
                                        validate: value => validateMaxGreaterThanMin(value, 'averageSlmTokens'),
                                    })}
                                    isDestructive={!!errors.averageSlmTokens?.max?.message}
                                    placeholder="Max"
                                    type="number"
                                    onInput={handleInputChange}
                                />
                            </div>
                            {errors?.averageSlmTokens?.max && (
                                <p className="text-red-500 text-xs">{errors.averageSlmTokens.max.message}</p>
                            )}
                        </div>
                        <div className="flex flex-col gap-y-2">
                            <div>
                            ...
                            <div className="flex items-center gap-x-3">
                                <Input
                                    {...register('mostSlmTokens.min', { valueAsNumber: true })}
                                    placeholder="Min"
                                    type="number"
                                    onBlur={() => onMinValueChange('mostSlmTokens')}
                                    onInput={handleInputChange}
                                />
                                <Input
                                    {...register('mostSlmTokens.max', {
                                        valueAsNumber: true,
                                        validate: value => validateMaxGreaterThanMin(value, 'mostSlmTokens'),
                                    })}
                                    isDestructive={!!errors.mostSlmTokens?.max?.message}
                                    placeholder="Max"
                                    type="number"
                                    onInput={handleInputChange}
                                />
                            </div>
                            {errors?.mostSlmTokens?.max && (
                                <p className="text-red-500 text-xs">{errors.mostSlmTokens.max.message}</p>
                            )}
                        </div>
                        <div className="flex flex-col gap-y-2">
                            <div>
...
export const SLMTimeTable = ({ slmExecutions, onSLMExecutionFilter }: SLMTimeTableProps) => {
    return (
        <DataTable
            columns={columns}
            data={slmExecutions}
            ...
            tableHeader={
                <div className="flex justify-between items-center w-full">
                    <p className="text-sm font-bold text-gray-700 dark:text-gray-200">SLM Time</p>
                    <SLMTimeFilter onSLMExecutionFilter={onSLMExecutionFilter} />
                </div>
            }
        />
    );
